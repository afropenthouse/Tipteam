...
import { Copy, Download, MapPin, Mail, Phone, Star, Upload, FileText, X, Edit, UserCheck } from "lucide-react";
...
export default function BusinessDetail() {
  ...
  const handleEditClick = () => {
    if (business) {
      setEditForm({
        ...
        allowCheckin: business.allowCheckin ?? false
      });
  ...
  const download = async () => {
    try {
      ...
      a.href = pngData;
      a.download = `${business.name}-rating-qr.png`;
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
      
      toast({ title: "Download successful", description: "Rating QR code saved as PNG" });
    } catch (error) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-6 lg:grid-cols-2 items-start">
        <div className="rounded-2xl border bg-gradient-mint p-4 sm:p-6 shadow-card h-full">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-4">
            <div>
              <h2 className="font-semibold text-accent-foreground">Rating, Tips & Check-in QR</h2>
              <p className="mt-1 text-xs text-accent-foreground/80">
                One QR for everything. Customers scan this to leave feedback, tip, or check in.
              </p>
            </div>
            {business.allowCheckin && (
              <Button asChild variant="outline" size="sm" className="bg-white/50 border-accent-foreground/20 text-accent-foreground hover:bg-white/80 h-8 text-xs">
                <Link to={`/checkin/${id}`} target="_blank">
          ...
          <div className="flex flex-col items-center gap-6 rounded-xl bg-card p-4 sm:p-8">
            <div className="relative flex-shrink-0 w-full max-w-[240px]">
              <div className="absolute -top-5 sm:-top-6 left-0 right-0 flex justify-center z-10 px-2">
                <span className="inline-block text-black text-[10px] sm:text-xs font-bold px-2 sm:px-4 py-1 sm:py-2 whitespace-nowrap text-center bg-white/90 backdrop-blur-sm rounded-lg shadow-sm border border-gray-100">
                  Scan to Rate, Tip or Check In
                </span>
              </div>
              <div className="flex justify-center p-3 bg-white rounded-xl shadow-inner border border-gray-100">
                <QRCodeCanvas 
                  id="biz-qr" 
                  value={rateUrl} 
                  size={qrSize} 
            ...
            <div className="w-full max-w-md space-y-4">
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  <Label className="text-[10px] uppercase tracking-wider text-muted-foreground font-bold">Public Link</Label>
                  <Button onClick={copy} variant="ghost" size="sm" className="h-6 text-primary gap-1 text-[10px]">
                    <Copy className="h-3 w-3" /> Copy
                  </Button>
                </div>
                <div className="bg-muted/50 p-2.5 rounded-lg border border-gray-100 text-center">
                  <span className="text-xs break-all text-muted-foreground font-mono">
                    {rateUrl}
                  </span>
                </div>
              </div>

              <Button onClick={download} className="w-full h-11 bg-gradient-primary shadow-elegant font-bold">
        ...
        <div className="rounded-2xl border bg-card p-6 shadow-card h-full">
      ...
      <Dialog open={editModalOpen} onOpenChange={setEditModalOpen}>
        <DialogContent className="sm:max-w-[425px] max-h-[80vh] overflow-y-auto">
          ...
          <div className="grid gap-4 py-4">
            ...
            <div className="flex items-center space-x-3 p-4 border rounded-lg bg-muted/30">
              <Switch
                id="allow-checkin-edit"
                checked={editForm.allowCheckin}
                onCheckedChange={(checked) => handleFormChange("allowCheckin", checked)}
              />
              <div className="space-y-1">
